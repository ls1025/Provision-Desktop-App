...
Public Class locationmatrix
    ...
    Private Sub locationmatrix_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadData()
    ...
    Public Sub loadData()
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "select doctype,location,Count(location) as 'total' from archival_master where location IS NOT NULL group by doctype,location order by location ASC"
            con.Open()
            ...
            While dr.Read
                ...
                ' End If
                pic.Text = Environment.NewLine + dr.Item("location").ToString
                ...
                ProductLabel.Text = dr.Item("doctype").ToString
    ...
    Private Sub filterdata1(ValueTosearch As String)
        Try
            ...
            Dim cmd As New SqlCommand
            cmd.Connection = con
            cmd.CommandText = "select doctype,location,Count(location) as 'total' from archival_master where location IS NOT NULL and ([doctype] like '%" & ValueTosearch & "%' or [docname] like '%" & ValueTosearch & "%' or [docno] like '%" & ValueTosearch & "%' or [arcno] like '%" & ValueTosearch & "%' or [location] like '%" & ValueTosearch & "%') group by doctype,location order by location ASC"
            con.Open()
            Dim dr As SqlDataReader = cmd.ExecuteReader
    ...
    Public Sub Pnale_Show(sender As Object, e As EventArgs)
        Dim form As New documentsinlocation
