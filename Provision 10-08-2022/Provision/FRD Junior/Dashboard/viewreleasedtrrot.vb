...
Imports SautinSoft
Imports System.IO
Public Class viewreleasedtrrot
    Dim da As New SqlDataAdapter()

    Private Sub viewreleasedtrrot_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Public Sub LoadIntimationRot()
        Try
            ...
            Dim cmd As New SqlCommand("select trrot.id,trrot.productcode, trrot.strength,trrot.btn, trrot.trno,trrot.requestedby, trrot.requestdate,trrot.datauploaddate,viewtraudit.viewedby,viewtraudit.vieweddate 
from trrot
 OUTER APPLY (Select TOP 1 viewtraudit.viewedby,viewtraudit.vieweddate  from viewtraudit where viewtraudit.trno=trrot.trno order by vieweddate DESC)  viewtraudit
 
 where

			trrot.productname = '" + Label1.Text + "' AND 
				trrot.test = '" + Label2.Text + "' AND 
					trrot.datauploaddate BETWEEN @fromdate AND @todate  order by trrot.datauploaddate DESC
")
        ...
        If TypeOf senderGrid.Columns(e.ColumnIndex) Is DataGridViewButtonColumn AndAlso e.RowIndex >= 0 Then
            ...
            If e.ColumnIndex = 1 Then
                Try
                    If con.State = ConnectionState.Open Then con.Close()
                    Dim qry1 As String
                    qry1 = "select datafile from trrot where trno='" + DataGridView1.Rows(e.RowIndex).Cells(6).Value.ToString() + "'"
    ...
    Private Sub WithNoPrint()
        Try
            ...
            If con.State = ConnectionState.Open Then con.Close()
            Dim qry1 As String
            qry1 = "select datafile from trrot where trno="
            cmd = New SqlCommand(qry1, con)
            Dim sFilePath As String
            Dim buffer As Byte()
            con.Open()
