...
Public Class addproduct
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Try
            ...
            ElseIf GunaTextBox1.Text = "" Then
                MsgBox("Enter Client", vbCritical)
            ...
            Else
                ...
                Dim qry As String
                qry = "select COUNT(proname) from productmaster where proname='" + TextBox1.Text + "' and market='" + TextBox2.Text + "'"
                    ...
                    For i = 0 To DataGridView1.Rows.Count - 1
                        ...
                        If SameProductCode = 0 Then
                            If ProductAddedby = "User Type" Then
                                ...
                                cmd3.Connection = con
                                cmd3.CommandText = "insert into  productmaster(procode,proname,strength,market,client,label,productaddedby,productaddeddate,productstatus,pds,lnb) values('" + DataGridView1.Rows(i).Cells(0).Value + "','" + TextBox1.Text + "','" + DataGridView1.Rows(i).Cells(1).Value + "','" + TextBox2.Text + "','" + GunaTextBox1.Text + "','" + DataGridView1.Rows(i).Cells(2).Value + "','" + homepage.Label1.Text + "',@productaddeddate,'Active','No','No')"
                            ...
                            Else
                                ...
                                Dim cmd3 As New SqlCommand
                                cmd3.Connection = con
                                cmd3.CommandText = "insert into  productmaster(procode,proname,strength,market,client,label,productaddedby,productaddeddate,productstatus,pds,lnb) values('" + DataGridView1.Rows(i).Cells(0).Value + "','" + TextBox1.Text + "','" + DataGridView1.Rows(i).Cells(1).Value + "','" + TextBox2.Text + "','" + GunaTextBox1.Text + "','" + DataGridView1.Rows(i).Cells(2).Value + "','" + homepagedqa.Label1.Text + "',@productaddeddate,'Active','No','No')"
                                cmd3.Parameters.AddWithValue("@productaddeddate", SqlDbType.Date).Value = DateTime.Now.Date
                                con.Open()
                                cmd3.ExecuteNonQuery()
                                con.Close()
                            ...
                            End If
                        Else
                            MsgBox("Product Code " + DataGridView1.Rows(i).Cells(0).Value + " is Already Exist", vbCritical)
                            Exit Sub
                        End If
                    Next
                    MsgBox("Prodcut Added Successfully", vbInformation)
                    Dim f1 As viewproductmasters = DirectCast(Me.Owner, viewproductmasters)
                    f1.LoadProductMaster()
                    Me.Close()
                Else
                    MsgBox("Product Name with Market Already Exist", vbCritical)
                    Exit Sub
                End If
    ...
    Private Sub Insert_ProductInitiation_Form()
        Try
            ...
            Dim cmd3 As New SqlCommand
            cmd3.Connection = con
            cmd3.CommandText = "insert into productinitiationform(proname,market,client,attachmentname,attachment) values('" + TextBox1.Text + "','" + TextBox2.Text + "','" + GunaTextBox1.Text + "','" + Path.GetFileName(_filename) + "',@attachment)"
            cmd3.Parameters.AddWithValue("@attachment", SqlDbType.VarBinary).Value = File.ReadAllBytes(GunaLabel7.Text)
            con.Open()
            cmd3.ExecuteNonQuery()
            con.Close()
        Catch ex As Exception
