...
Public Class viewatrissuancelogard
    ...
    Private Sub viewatrissuancelogard_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        loadData()
    End Sub
    ...
    Private Sub filterdata1(ValueTosearch As String)
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "Select proname,test,atrissueno,issueddate from atrissuelog where [atrno] IS NOT NULL and [proname] like '%" & ValueTosearch & "%' or [test] like '%" & ValueTosearch & "%' or [trno] like '%" & ValueTosearch & "%' or [atrissueno] like '%" & ValueTosearch & "%' Group by proname,test,atrissueno,issueddate"
            con.Open()
    ...
    Public Sub loadData()
        Try
            ...
            Dim cmd As New SqlCommand
            cmd.Connection = con
            cmd.CommandText = "Select proname,test,atrissueno,issueddate from atrissuelog where [atrno] IS NOT NULL Group by proname,test,atrissueno,issueddate order by issueddate DESC"
            con.Open()
            Dim dr As SqlDataReader = cmd.ExecuteReader
