...
Imports SautinSoft
Imports System.IO
Public Class viewreleasedtrdev
    Dim da As New SqlDataAdapter()

    Private Sub viewreleasedtrdev_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Public Sub LoadIntimationDev()
        Try
            ...
            Dim cmd As New SqlCommand("select trdev.id,trdev.productcode, trdev.strength,trdev.btn, trdev.trno,trdev.requestedby, trdev.requestdate,trdev.datauploaddate,viewtraudit.viewedby,viewtraudit.vieweddate 
from trdev
 OUTER APPLY (Select TOP 1 viewtraudit.viewedby,viewtraudit.vieweddate  from viewtraudit where viewtraudit.trno=trdev.trno order by vieweddate DESC)  viewtraudit
 
 where

			trdev.productname = '" + Label1.Text + "' AND 
				trdev.test = '" + Label2.Text + "' AND 
					trdev.datauploaddate BETWEEN @fromdate AND @todate  order by trdev.datauploaddate DESC
")
        ...
        If TypeOf senderGrid.Columns(e.ColumnIndex) Is DataGridViewButtonColumn AndAlso e.RowIndex >= 0 Then
            ...
            If e.ColumnIndex = 1 Then
                Try
                    If con.State = ConnectionState.Open Then con.Close()
                    Dim qry1 As String
                    qry1 = "select datafile from trdev where trno='" + DataGridView1.Rows(e.RowIndex).Cells(6).Value.ToString() + "'"
    ...
    Private Sub WithNoPrint()
        Try
            ...
            If con.State = ConnectionState.Open Then con.Close()
            Dim qry1 As String
            qry1 = "select datafile from trdev where trno=''"
            cmd = New SqlCommand(qry1, con)
            Dim sFilePath As String
            Dim buffer As Byte()
            con.Open()
