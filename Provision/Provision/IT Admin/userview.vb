...
Public Class userview
    ...
    Public Sub LoadUserDetails()
        Try
            ...
            cmd.Connection = con
            da = New SqlDataAdapter("Select [id],[fullname],[username],[type],[reportto],[status] from users order by [fullname] ASC", con)
    ...
    Private Sub filterdata1(ValueTosearch As String)
        Try
            If con.State = ConnectionState.Open Then con.Close()
            Dim cmd As New SqlCommand("Select [id],[fullname],[username],[type],[reportto],[status] from users  where ([fullname] like '%" & ValueTosearch & "%' or [username] like '%" & ValueTosearch & "%') order by [fullname] ASC")
            cmd.Connection = con
    ...
    Private Sub GunaGradientButton2_Click(sender As Object, e As EventArgs) Handles GunaGradientButton2.Click
        If DataGridView1.SelectedCells.Count > 0 Then
            ...
            ' form.ComboBox1.Text = DataGridView1.SelectedCells(3).Value.ToString
            ' form.ComboBox2.Text = DataGridView1.SelectedCells(4).Value.ToString
            '  If DataGridView1.SelectedCells(5).Value = "Active" Or DataGridView1.SelectedCells(5).Value = "Renew" Then
