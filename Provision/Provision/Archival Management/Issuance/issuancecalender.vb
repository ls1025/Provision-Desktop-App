...
Public Class issuancecalender
    Private listFlDay As New List(Of FlowLayoutPanel)
    Private currentDate As DateTime = DateTime.Today
    Private Sub issuancecalender_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        GenerateDayPanel(42)
        DisplayCurrentDate()
    ...
    Private Sub AddAppointmentToFlDay(ByVal startDayAtFlNumber As Integer)
        Try
            ...
            ' Dim sql1 As String = "select COUNT(*) AS Total from archival_issuance where issueddate<=DATEADD(DAY,-15,GETDATE())"

            'Dim sql As String = ""
            'Dim da As New SqlDataAdapter(sql, con)
            ' Dim dt As DataTable = QueryAsDataTable1(sql)

            Dim cmd As New SqlCommand("select COUNT(*) as 'Total', DATEADD(DAY,15,issueddate) AS 'duedate' from archival_issuance where DATEADD(DAY,15,issueddate) between '" + startDate.ToString("dd MMM yyyy") + "' and '" + endDate.ToString("dd MMM yyyy") + "' group by issueddate")
            cmd.Connection = con
            ...
            For Each row As DataRow In table1.Rows
                Dim appDay As DateTime = DateTime.Parse(row("duedate"))

                Dim lab As New Label
                lab.Tag = row("duedate")
                lab.Name = row("duedate")
