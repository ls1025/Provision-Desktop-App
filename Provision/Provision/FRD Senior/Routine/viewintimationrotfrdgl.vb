...
Imports iTextSharp
Imports System.IO
Public Class viewintimationrotfrdgl
    Dim da As New SqlDataAdapter()
    Private Sub viewintimationrotfrdgl_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Private Sub LoadProductNameRot()
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "select DISTINCT(productname) from trrot order by [productname] ASC"
            con.Open()
    ...
    Private Sub LoadTestRot()
        Try
            ...
            Dim cmd As New SqlCommand
            cmd.Connection = con
            cmd.CommandText = "select DISTINCT(test) from trrot order by [test] ASC"
            con.Open()
            Dim dr As SqlDataReader = cmd.ExecuteReader
            While dr.Read
        ...
        End Try
    End Sub
    Public Sub LoadIntimationRot()
        Try

            If con.State = ConnectionState.Open Then con.Close()
            Dim cmd As New SqlCommand("Select [ID],[productcode],[productname],[strength],[btn],[test],[trno],[requestedby],[requestdate],[Status] from trrot order by [trno] DESC")
            cmd.Connection = con
    ...
    Private Sub GunaGradientButton1_Click(sender As Object, e As EventArgs) Handles GunaGradientButton1.Click
        Dim form As New addtrrotfrdgl
        form.Owner = Me
        form.ShowDialog()
    End Sub
    Private Sub GunaGradientButton2_Click(sender As Object, e As EventArgs) Handles GunaGradientButton2.Click

        Dim form As New viewcancelintrotfrdgl
        form.Owner = Me
        form.ShowDialog()
    End Sub
    ...
    Private Sub Filterdata1(ValueTosearch As String)
        Try
            If con.State = ConnectionState.Open Then con.Close()
            Dim cmd As New SqlCommand("Select [ID],[productcode],[productname],[strength],[btn],[test],[trno],[requestedby],[requestdate],[Status] from trrot where ([trno] like '%" & ValueTosearch & "%' or [productcode] like '%" & ValueTosearch & "%' or [btn] like '%" & ValueTosearch & "%' or [Status] like '%" & ValueTosearch & "%') order by [trno] DESC")
            cmd.Connection = con
            Dim adapter1 As New SqlDataAdapter(cmd)
            Dim table1 As New DataTable()
            DataGridView1.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.None
    ...
    Private Sub ComboBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox1.SelectedIndexChanged
        ...
        Try
            If ComboBox2.Text = "" Then
                If con.State = ConnectionState.Open Then con.Close()
                Dim cmd As New SqlCommand("Select [ID],[productcode],[productname],[strength],[btn],[test],[trno],[requestedby],[requestdate],[Status] from trrot where [productname] = '" + ComboBox1.Text + "' order by [trno] DESC")
                cmd.Connection = con
                Dim adapter1 As New SqlDataAdapter(cmd)
                Dim table1 As New DataTable()
                DataGridView1.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.None
            ...
            ElseIf ComboBox2.Text <> "" Then
                If con.State = ConnectionState.Open Then con.Close()
                Dim cmd As New SqlCommand("Select [ID],[productcode],[productname],[strength],[btn],[test],[trno],[requestedby],[requestdate],[Status] from trrot where [productname] = '" + ComboBox1.Text + "' and [test]='" + ComboBox2.Text + "' order by [trno] DESC")
                cmd.Connection = con
                Dim adapter1 As New SqlDataAdapter(cmd)
                Dim table1 As New DataTable()
                DataGridView1.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.None
    ...
    Private Sub ComboBox2_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox2.SelectedIndexChanged
        Try
            If ComboBox1.Text = "" Then
                If con.State = ConnectionState.Open Then con.Close()
                Dim cmd As New SqlCommand("Select [ID],[productcode],[productname],[strength],[btn],[test],[trno],[requestedby],[requestdate],[Status] from trrot where [test] = '" + ComboBox2.Text + "' order by [trno] DESC")
                cmd.Connection = con
                Dim adapter1 As New SqlDataAdapter(cmd)
                Dim table1 As New DataTable()
                DataGridView1.AutoSizeRowsMode = DataGridViewAutoSizeRowsMode.None
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        ElseIf ComboBox3.Text = "Intimation" Then
            Try
                If con.State = ConnectionState.Open Then con.Close()
                Dim cmd As New SqlCommand("Select [ID],[productcode],[productname],[strength],[btn],[test],[trno],[requestedby],[requestdate],[Status] from trrot where requestdate between @fromdate and @todate order by [trno] DESC")
                cmd.Connection = con
        ...
        ElseIf ComboBox3.Text = "Released" Then
            Try
                If con.State = ConnectionState.Open Then con.Close()
                Dim cmd As New SqlCommand("Select [ID],[productcode],[productname],[strength],[btn],[test],[trno],[requestedby],[requestdate],[Status] from trrot where releaseddate between @fromdate and @todate order by [trno] DESC")
                cmd.Connection = con
                cmd.Parameters.AddWithValue("@fromdate", SqlDbType.Date).Value = DateTimePicker1.Value.Date
                cmd.Parameters.AddWithValue("@todate", SqlDbType.Date).Value = DateTimePicker2.Value.Date
                Dim adapter1 As New SqlDataAdapter(cmd)
