...
Imports System.IO
Public Class viewstabilityprotocoldqa
    Private WithEvents pic As New Guna.UI.WinForms.GunaGradientButton
    Dim da As New SqlDataAdapter()
    Dim dt As New DataTable()
    Private Sub viewstabilityprotocoldqa_Load(sender As Object, e As EventArgs) Handles Me.Load
    ...
    Public Sub LoadProtocols()
        Try
            ...
            cmd.Connection = con
            cmd.CommandText = "Select proname,strength,btn,ptn,status from stability_protocol where status='Submitted for Review' group by proname,strength,btn,ptn,status order by proname ASC"
            con.Open()
    ...
    Private Sub filterdata1(ValueTosearch As String)
        Try
            ...
            Dim cmd As New SqlCommand
            cmd.Connection = con
            cmd.CommandText = "Select proname,strength,btn,ptn,status from stability_protocol where status='Submitted for Review' and [proname] like '%" & ValueTosearch & "%' or [btn] like '%" & ValueTosearch & "%' or [ptn] like '%" & ValueTosearch & "%' Group by proname,strength,btn,ptn,status order by proname ASC"
            con.Open()
            Dim dr As SqlDataReader = cmd.ExecuteReader
    ...
    Private Sub ViewDetailStabilityPlanner(sender As Object, e As EventArgs)
        ...
        Dim form As New detailviewprotocoldqa
